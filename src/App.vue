...
class Masu {
    ...
    equal(masu) { return ((masu.suji === this.suji) && (masu.dan === this.dan)); }
}

// 盤上にすべてのマスを含むクラス
class MasuList {
    constructor() {
        this.reflist = ref([]); // Masuクラスのオブジェクトのリスト
        for (let s = 1; s <= 9; s++) {
            for (let d = 1; d <= 9; d++) {
...
// 駒クラス
class Koma {
    constructor(name, senteFlag, nariFlag) {
...
// 盤上にある駒クラス
class BanKoma {
    constructor(koma, masu) {
        this.koma = koma; // 駒 (Komaクラスのオブジェクト)
        this.masu = masu; // マス (Masuクラスのオブジェクト)
    }

    at(masu) { return this.masu.equal(masu); }  // BanKoma が指定のマスにあるかどうか

    // 駒を新しいマスに動かす
    moveTo(newMasu) { this.masu = newMasu; }
...
class BanKomaList {
    ...
    getIndexAt(masu) { return this.getList().findIndex((bk) => bk.at(masu)); }

    // 指定した位置に駒があるかどうかを返す
    hasKomaAt(masu) { return (this.getIndexAt(masu) != -1); }
    ...
    newClick(event, masu) {
        if (this.hasKomaAt(masu)) {
            ...
            this.setElemClass(event.target);
        }
    }
...
// すべての持駒を含むクラス (先手、後手別)
class MochiKomaList {
    constructor(senteFlag) {
        this.reflist = ref([]); // MochiKomaクラスのオブジェクトのリスト
        this.senteFlag = senteFlag; // 先手の持駒かどうか
        this.preClickIndex = -1; // ひとつ前にクリックされた持駒の index
        this.preClickHTMLElem = null;  // ひとつ前にクリックされた HTML element
    }
...
class RightClickMenu {
    constructor() {
        ...
        this.x = ref(0);
        this.y = ref(0);
        this.komaSengoGyaku = null; // 右クリックした駒を先後逆にした駒 (koma)
        this.komaSengoGyakuId = ref(""); // 先後逆にした駒の symbolid
        this.komaUra = null; // 右クリックした駒を裏返した駒 (koma)
        this.komaUraId = ref(""); // 裏側にした駒の symbolid
        this.komaHasUra = ref(true); // 裏側がある駒かどうか
    }

    // メニューの位置とマスと駒をセットする。
    set(targetElem, masu, koma) {
...
function clickSenteKomadai() {
    if (banKomaList.wasClicked()) {
        // 盤から駒台に駒を追加
        senteMochiKomaList.add(banKomaList.pickPreClickKoma());
...
function clickGoteKomadai() {
    if (banKomaList.wasClicked()) {
        // 盤から駒台に駒を追加
        goteMochiKomaList.add(banKomaList.pickPreClickKoma());
...
function clickSenteMochiKoma(event, index) {
    resetClickAll();
    senteMochiKomaList.newClick(event, index);
}

// 後手の駒台の持駒がクリックされたときの処理
function clickGoteMochiKoma(event, index) {
    resetClickAll();
...
function clickGomibako() {
    if (banKomaList.wasClicked()) {
        // 駒台に駒を追加
        gomibakoKomaList.add(banKomaList.pickPreClickKoma());
    } else if (senteMochiKomaList.wasClicked()) {
        // 先手の持駒から使わない駒置き場へ移動
...
function clickGomibakoKoma(event, index) {
    resetClickAll();
    gomibakoKomaList.newClick(event, index);
}

// マスがクリックされたときの処理
function clickMasu(event, masu) {
    // ひとつ前にマスがクリックされていた場合
    if (banKomaList.wasClicked()) {
...
// マスが右グリックされたときの処理
function rightClickMasu(event, masu) {
    resetClickAll();
...
function clickSetSengoGyaku() {
    banKomaList.remove(rightClickMenu.getMasu());
    banKomaList.add(rightClickMenu.getKomaSengoGyaku(), rightClickMenu.getMasu());
...
function clickSetUra() {
    banKomaList.remove(rightClickMenu.getMasu());
    banKomaList.add(rightClickMenu.getKomaUra(), rightClickMenu.getMasu());
    rightClickMenu.hideMenu();
}
